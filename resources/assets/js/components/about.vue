<template lang="html">
  <div id="aboutus">
  <nav>
    <div class="navbar navbar--fixed">
      <div class="navbar--blue">
        <div class="navbar--items-text">
          <ul>
            <li><a href="#" class="selected">About</a></li>
            <li><a href="#">Resource</a></li>
            <li><a href="#">Style</a></li>
          </ul>
        </div>
      </div>
    </div>
  </nav>
<section>
  <main>
    <div class="about1">
      <div class="wrapper-about1">
        <div class="about1-header">
          <h1>It's about</h1>
          <h1>Build a Teamwork</h1>
        </div>
        <div class="text--grey-detail">
          <p>Interesting team management , its will help you and your team very much. Its a semi-git and a fusioner web , octolist can do much as u want. Creating your team , creating project , meeting with your friend. </p>
        </div>
        <div class="blue-panel"></div>
      </div>
    </div>
    <div class="about2">
      <div class="wrapper-about1">
        <div class="about2-header marginMagic textAlignCenter">
          <h3>Start a journey with</h3>
          <h2>Creating boards</h2>
        </div>
        <div class="pic-user">
          <div class="avatar avatar--xl avatar--glow pic-user-1">
            <img src="/images/avatar0.jpg" alt="" class="avatar-img">
          </div>
          <div class="avatar avatar--m avatar--glow pic-user-2">
            <img src="/images/kecil6.jpg" alt="" class="avatar-img">
          </div>
          <div class="avatar avatar--l avatar--glow pic-user-3">
            <img src="/images/kecil5.jpg" alt="" class="avatar-img">
          </div>
        </div>
        <div class="desc-about2">
          <p>Plan projects and manage resources with ease. Roadmap helps you get the most out of your projects and teams.</p>
        </div>
        <div class="content" style="min-height: auto; height: 0;">
          <div class="content__items">
            <div class="project">
              <div class="project__content" style="justify-content: center;">
                <div class="project-item" style="border: none;">
                  <div class="project-item__header">
                    <span class="overlay-dark">
                      <a href="javascript:void(0)">
                        <img class="icon" src="images/edit.svg" alt="" />
                      </a>
                    </span>
                    <img src="images/projectsample.jpg" alt="" />
                  </div>
                  <div class="project-item__content">
                    <h5>
                      Typography Culture
                    </h5>
                    <div class="content-red">
                      <div class="content-desc">
                        <div class="content-people">
                          <img src="images/kecil1.png" alt="" />
                          <img src="images/kecil6.jpg" alt="" />
                          <img src="images/kecil5.jpg" alt="" />
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="project-item__footer">
                    <div class="project-item__footer-items">
                      <span>Open project</span>
                    </div>
                  </div>
              </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="about3">
      <div class="wrapper-about1">
        <div class="about3-header">
          <h1>Complex</h1>
          <h1 style="top: 140px; left: 20px; position: absolute;color: white;">Complex</h1>
        </div>
        <div class="about3-desc">
          <p>We have complex features on Octolist. Its will help your team and urself ofcourse to improve teamwork , Build your project like build blocks.</p>
        </div>
      </div>
    </div>
    <div class="about4">
      <div class="wrapper-about1">
        <div class="about4-header">
          <h2>Fresh design</h2>
        </div>
      </div>
    </div>
  </main>
</section>
</div>
</template>
<script src="" charset="utf-8"></script>
<script>
export default {
}
</script>

<style lang="css">
</style>
